...
import SellectUsers from "./SellectUsers";
import { useRoute } from "@react-navigation/native";

//main function
const UpdateGroup = ({ navigation }: any) => {
  ...
  const [group, setGroup] = useState<ConvProps | null>(null);
  ...
  useEffect(() => {
    ...
    if (route.params?.group) {
      ...
      setSeletecdUsers(route.params?.group?.users);
      setChatName(route.params?.group?.chatName);
      setPhoto({ uri: route.params?.group?.image, type: "image" });
    }
  }, [route.params]);
