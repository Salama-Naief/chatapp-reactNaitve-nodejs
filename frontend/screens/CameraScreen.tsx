...
const CameraScreen = ({ navigation }: any) => {
  ...
  const [vedioRecordMode, setVedioRecordMode] = useState<boolean>(false);
    ...
    if (mode === "video") {
      if (camera.current) {
        try {
          ...
          setVedioRecordMode(true);
          console.log("video", video);
        } catch (err) {
          console.log("rroorr", err);
          setVedioRecordMode(false);
        }
      }
    } else {
      setMode("video");
        ...
        <View style={{ height, width }} className="bg-black relative ">
          ...
          <View className="flex-row absolute bottom-6 left-0 justify-between w-full p-4 bg-black">
            <TouchableOpacity onPress={() => setTackedImage(null)}>
              <Ionicons name="repeat-outline" size={30} color={iconsColor} />
            </TouchableOpacity>
          ...
          <View className="flex-row justify-between p-4 flex-1">
            ...
            <View>
              {mode === "video" && (
                <Text
                  className={`${
                    vedioRecordMode ? "text-red-400" : "text-white"
                  }`}
                >
          ...
          <View className="">
            ...
            <View className="flex-row justify-between px-4 pb-10">
              <TouchableOpacity onPress={() => loadPictures()}>
                <Ionicons name="images-outline" size={26} color={iconsColor} />
              </TouchableOpacity>
              <View
